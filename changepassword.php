...
if($_POST)
{
	...
	$sq = mysqli_query($con,"select * from tbl_usermaster where user_email='{$_SESSION['email']}' ") or die("Error sq".mysqli_error($con));
	$count = mysqli_num_rows($sq);
	$fr = mysqli_fetch_array($sq);

	if($fr['user_password'] == $opass)
    {
        if($npass == $cpass)
        {
            if($opass == $npass)
            ...
            {
                $uq = mysqli_query($con,"update tbl_usermaster set user_password='{$npass}' where user_email='{$_SESSION['email']}'")or die("Error in updateq".mysqli_error($con));
                if($uq)
                {
						...
						<div class="col-md-8">
							<div class="box">
								...
								<form method="POST" role="form">
                                    ...
                                    <button type="submit" onclick="window.location='home.php';" class="btn btn-primary btn-inline">Back</button>&nbsp; &nbsp; &nbsp;
								</form>
							</div>
						</div>
