...
{
	$bid = $_GET['bid'];
	$statusq = mysqli_query($con,"select status from tbl_bookingmaster where book_id='{$bid}'")or die(mysqli_error($con));
	$statusfr=mysqli_fetch_array($statusq);

	if($statusfr['status'] == 1)
	{
		$uq = mysqli_query($con,"update tbl_bookingmaster set status=0 where book_id='{$bid}'")or die(mysqli_error($con));

	}else
	{
		$uq = mysqli_query($con,"update tbl_bookingmaster set status=1 where book_id='{$bid}'")or die(mysqli_error($con));
	}
}
?>
		...
		<header class="header header-menu-fullw">
			...
			<div class="header-main">
				<div class="container">
					<!-- Logo -->
					<div class="logo">
						<a href="home.php"><img src="images/logo.png" alt="Handyman"></a>
					...
					<nav class="nav-main">
						<div class="nav-main-inner">
							<ul data-breakpoint="992" class="flexnav">
								<li><a href="workerchangepassword.php">Changepassword</a></li>
						...
						<div class="table-responsive">
							<table class="job-manager-jobs table table-bordered table-striped">
								...
								<?php 
									$i = 0;
									$grandtotal=array();
									$bsq = mysqli_query($con,"select * from tbl_bookingmaster where worker_id='{$_SESSION['wid']}'")or die("Error Bsq".mysqli_error($con));
									...
									while($bfr = mysqli_fetch_array($bsq))
									{
										...
										$i++;
											$usq = mysqli_query($con,"select user_name from tbl_usermaster where user_id='{$bfr['user_id']}'")or die("error WNSQ".mysqli_error($con));
											$ufr = mysqli_fetch_array($usq);
											$status = $bfr['status'];
											if($status == 1)
											{
												$status="<a href='?bid={$bfr['book_id']}'>Pending</a>";
											}else
											{
												$status="<a href='?bid={$bfr['book_id']}'>Done</a>";
											}
